...
class dataUploadView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            form.save()
            data_bg= request.POST.get('Glucose')
            data_bp=request.POST.get('BloodPressure')
            data_sk=request.POST.get('SkinThickness')
            data_ins=request.POST.get('Insulin')
            data_bmi=request.POST.get('BMI')
            data_dpf=request.POST.get('DiabetesPedigreeFunction')
            data_preg=request.POST.get('Pregnancies')
            ...
            data = np.array([float(data_bg),float(data_bp), float(data_sk), float(data_ins), float(data_bmi),float(data_dpf),float(data_preg),float(data_age)])
            #sc = StandardScaler()
...
#data_for_prediction =obj.pca(np.array(data_for_prediction),y_test)
            ...
            return render(request, "succ_msg.html", {'data_bg':data_bg,'data_bp':data_bp,'data_sk':data_sk,'data_ins':data_ins,'data_bmi':data_bmi,'data_dpf':data_dpf,'data_preg':data_preg,'data_age':data_age,
                                                        'out':out})
